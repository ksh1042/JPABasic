...
> commit 발생 전, 사용자 정의 쿼리가 발생할 경우에 flush가 수행된다.<br>
...
>- 임베디드 타입을 생성할 경우 인스턴스 생성당시를 제외한 데이터의 조작이 불가능하도록, 기본 생성자와 ```Setter``` 메소드를 생성하지 말아야 한다. 
>- 다만 기본생성자가 없을 경우 JPA 스펙에 맞지 않아 컴파일 오류가 발생하므로 빈 기본 생성자를 생성하되 ```protected``` 접근지정자를 설정하여 외부에서 호출하는 것을 금지한다. (```private``` 접근지정자는 JPA 스펙에 맞지 않아 오류가 발생한다) 
> ```java
> @Embeddable
> public class Emb
> {
...
- ```Criteria```
>- JPQL Builder 역할로 사용법이 어려워 습득기간이 오래걸리며 유지보수가 어려워지는 코드가 자주 발생하니 추후에 필요할때 학습할 것.
>- 굳이 쓴다면 QueryDSL을 사용하는 것을 추천
...
- 엔티티 필드 컬렉션 초기화
> 엔티티 필드는 엔티티 인스턴스 생성시 초기화 해주는 편이 좋다.
>1. NullPointerException 예방에 도움이 된다
>2. hibernate 구현체를 사용할 때에 영속화 된 컬렉션을 조회할 경우, 영속화 된 컬렉션을 추적하기 위해 하이버네이트가 별도의 클래스로 래핑해서 사용하므로 중간에 컬렉션 인스턴스를 변경 시 오동작할 우려가 있다.
> 
<hr/>
